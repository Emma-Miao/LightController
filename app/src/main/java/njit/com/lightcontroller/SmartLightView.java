...
public class SmartLightView extends View {
    ...
    private void initPaint() {                 //初始化画笔
        ...
        mPaintLight.setColor(getResources().getColor(R.color.colorCircle));//设置画笔颜色
        mPaintLight.setAntiAlias(true);   //开启抗锯齿功能
        mPaintLight.setStrokeWidth(15f);//设置灯泡画笔的宽度
        mPaintLight.setStrokeCap(Paint.Cap.SQUARE);//设置灯泡画笔的线冒样式
        mPaintLight.setStyle(Paint.Style.STROKE);//设置灯泡画笔样式为仅描边

        mPaintCircle = new Paint();
        mPaintCircle.setColor(getResources().getColor(R.color.colorCircle));
        mPaintCircle.setAntiAlias(true);
        mPaintCircle.setStyle(Paint.Style.FILL_AND_STROKE);  //设置圆环画笔样式为填充内部和描边
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isLightOn) {
            ...
            for (int i = 0; i < 5; i++) {             //画灯泡周围的圆环的涟漪效果
            ...
            if (progressShow < 30) {
                mPaintLight.setAlpha(80);   //设置透明度为80
            }
    ...
    @Override
    public boolean onTouchEvent(MotionEvent event) {        //点击smartlightview区域后运行

        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            if (getLightStatus()) {
                setLightOn(false);
                setBackgroundResource(R.drawable.galaxy);
    ...
    public synchronized void setProgressLumin(int progressLumin) {      //设置亮度条的进度
    ...
    public synchronized void setProgressTemper(int tempProgressTemper) {     //设置冷暖条的进度
